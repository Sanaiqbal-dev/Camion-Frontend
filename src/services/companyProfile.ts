...
export const companyProfile = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getCompanyProfilesList: builder.query<IAPIResponse<ICompanyProfile[]>, any>({
      query: (queryParams) => `/Account/GetCompanyVerificationUsers${queryParams !== null ? '?' + CreateQueryParams(queryParams) : ''}`,
      providesTags: ['Company'],
    }),

    updateCompanyAccount: builder.mutation<IAPIResponse<ICompanyProfile>, Partial<ICompanyProfile>>({
      query: (body) => ({
...
export const { useCreateCompanyProfileMutation, useGetCompanyProfilesListQuery, useUpdateCompanyAccountMutation } = companyProfile;
