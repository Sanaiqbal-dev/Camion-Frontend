...
import { IOrder, IOrderIndex,IOrderSingle, OrderIndexQuery } from '@/interface/order';
import baseApi from './baseApi';

export const orderApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    getOrders: builder.query<IAPIResponse<IOrderIndex[]>, any>({
      query: (queryParams) => `order/list${(queryParams !== null ? '?' + CreateQueryParams(queryParams): '')}`,
      providesTags: ['Order'],
    }),
...
// Export hooks for use in the app
	 export const { useGetOrdersQuery, useGetOrderQuery, useGetSelectOrdersQuery, 
	 								useAddOrderMutation, useUpdateOrderMutation, useDeleteOrderMutation} = orderApi;