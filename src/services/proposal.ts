...
export const proposalApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    getProposals: builder.query<IAPIResponse<IProposalIndex[]>, any>({
      query: (queryParams) => `proposal/list${(queryParams !== null ? '?' + CreateQueryParams(queryParams): '')}`,
    ...
    getProposal: builder.query<IAPIResponse<IProposalSingle>, Partial<IProposal> >({
      ...
      providesTags: (result, error, arg) => {
		...
		updateProposal: builder.mutation<IAPIResponse<IProposal>, Partial<IProposal>>({
			query: ({ id, ...rest }) => ({
				url: `proposal/${id}`,
				method: 'PUT',
				body: { id, ...rest },
